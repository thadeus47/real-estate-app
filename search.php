...
if(isset($_POST['h_search'])){
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE address LIKE '%{$h_address}%' AND type LIKE '%{$h_type}%' AND offer LIKE '%{$h_offer}%'  ORDER BY creation_time DESC");
...
//    $select_properties = $conn->prepare("SELECT * FROM `property` WHERE address LIKE '%{$address}%' AND type LIKE '%{$type}%' AND offer LIKE '%{$offer}%'  AND status LIKE '%{$status}%' AND furnished LIKE '%{$furnished}%'  ORDER BY creation_time DESC");
//    $select_properties->execute();

// }
else{
   $select_properties = $conn->prepare("SELECT * FROM `property` ORDER BY creation_time DESC LIMIT 6");
   $select_properties->execute();
}
